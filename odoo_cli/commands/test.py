...
def test(
    ...
    Tests run against `{database}-test`, recreated from scratch each run
    (--keep-db reuses it instead). Manifest-declared python dependencies
    missing from the venv are installed first (Odoo's requirements.txt
    omits some on purpose).
    """
    if as_json:
    ...
    ctx.services.testing.run(target, module_spec, list(tags), keep_db=keep_db)
    if as_json:
